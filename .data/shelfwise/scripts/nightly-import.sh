...
for TARGET_DATE in "${DATES_TO_GENERATE[@]}"; do
    ...
    psql -q -c "\COPY (SELECT customer_id, email, phone, first_name, last_name, age_bracket, household_size, income_bracket, primary_store_id, primary_city, home_latitude, home_longitude, CASE WHEN loyalty_member THEN 'true' ELSE 'false' END as loyalty_member, loyalty_tier, loyalty_join_date, preferred_shopping_time, price_sensitivity, customer_segment, created_date, CASE WHEN has_online_account THEN 'true' ELSE 'false' END as has_online_account, CASE WHEN prefers_online THEN 'true' ELSE 'false' END as prefers_online FROM customers) TO '$REFERENCE_DIR/customers.csv' WITH CSV HEADER"
    psql -q -c "\COPY (SELECT address_id, customer_id, address_type, CASE WHEN is_default THEN 'true' ELSE 'false' END as is_default, street_address, apartment_unit, city, state, zip_code, latitude, longitude, delivery_instructions, CASE WHEN has_doorman THEN 'true' ELSE 'false' END as has_doorman, CASE WHEN requires_signature THEN 'true' ELSE 'false' END as requires_signature, REPLACE(created_at::text, ' ', 'T') as created_at FROM customer_addresses) TO '$REFERENCE_DIR/customer_addresses.csv' WITH CSV HEADER"
    psql -q -c "\COPY (SELECT driver_id, first_name, last_name, phone, email, driver_type, employment_status, primary_store_id, service_radius_miles, service_cities, vehicle_type, vehicle_capacity_items, CASE WHEN has_insulated_bags THEN 'true' ELSE 'false' END as has_insulated_bags, acceptance_rate, CASE WHEN is_available THEN 'true' ELSE 'false' END as is_available, current_latitude, current_longitude, REPLACE(last_location_update::text, ' ', 'T') as last_location_update, hire_date, base_pay_per_delivery, mileage_rate FROM delivery_drivers) TO '$REFERENCE_DIR/delivery_drivers.csv' WITH CSV HEADER"
