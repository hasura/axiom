...
-- Drop tables (alphabetical, CASCADE handles dependencies)
...
BEGIN; TRUNCATE brands; COPY brands FROM '/docker-entrypoint-initdb.d/brands.csv' WITH (FORMAT csv, HEADER true, FREEZE); COMMIT;
...
BEGIN; TRUNCATE promotions; COPY promotions FROM '/docker-entrypoint-initdb.d/promotions.csv' WITH (FORMAT csv, HEADER true, FREEZE); COMMIT;
BEGIN; TRUNCATE promotion_types; COPY promotion_types FROM '/docker-entrypoint-initdb.d/promotion_types.csv' WITH (FORMAT csv, HEADER true, FREEZE); COMMIT;
...
ALTER TABLE delivery_assignments ADD CONSTRAINT fk_assignments_pickup_store
    FOREIGN KEY (pickup_store_id) REFERENCES stores(store_id) NOT VALID;
